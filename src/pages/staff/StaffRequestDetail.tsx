...
export default function StaffRequestDetail() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setRequest(prev => prev ? { ...prev, status, staffNotes: staffNotes } : null);
    ...
    <div className="min-h-screen cosmic-bg">
      <header className="border-b border-border bg-card/80 backdrop-blur-lg sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4 flex flex-wrap items-center gap-2 sm:gap-4">
          ...
          <span className="font-mono text-primary text-sm sm:text-base">{request.orderNumber}</span>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <Card className="glow-card">
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              {request.pickupWindow && <p className="flex items-center gap-2"><Clock className="h-4 w-4" />{request.pickupWindow}</p>}
